...
module setup
!
! Setup routine for uniform distribution under a static sinusoidal potential
! well to create a shock
!
! :References: None
!
! :Owner: Daniel Price
...
subroutine setpart(id,npart,npartoftype,xyzh,massoftype,vxyzu,polyk,gamma,hfact,time,fileprefix)
 ...
 use options,      only:icooling,alpha,alphau,nfulldump,iexternalforce,nmaxdumps
 ...
 use extern_staticsine, only:amplitude,wavek,phase,inclination
 ...
 real    :: vol_box,boxlength,cs0_cgs,deltax,rmsmach,v2i,turbfac,turbboxsize,turbamplitude,temp
 ...
 if (rms_mach > 0.) then
    ...
    call set_velfield_from_cubes(xyzh(:,1:npart),vxyzu(:,:npart),npart, &
                                 filex,filey,filez,turbamplitude,turbboxsize,.false.,ierr)
 ...
 if (.not. in_iexist) then
    ...
    iexternalforce = 13      ! 1-D static sinusoidal potential
    amplitude   = 80.
    wavek       = 2*pi/dxbound
    phase       = pi
    inclination = 0.
