...
subroutine setpart(id,npart,npartoftype,xyzh,massoftype,vxyzu,polyk,gamma,hfact,time,fileprefix)
 ...
 elseif (use_glass) then
    ...
    !
    open(3000,file='glassCube_64.dat',status='old',iostat=io)
    if (io < 0) call fatal('setup_unifdis_cmi','error opening glassfile')
    npart = 0
    do
       read(3000,*,iostat=io)
       if (io /= 0) then
