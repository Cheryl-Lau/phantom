...
def extract_shell_rad(ax):
    ...
    t0_shock = t0_shock_scf # Myr 
    firstcall = True

    for filename in glob.glob('semi_confined/vary_chnlsize/chnl005/shell_semiconf_1chnl005_*.dat'):  # cgs units 
        ...
        if (firstcall==True):
            ...
            ax.scatter(time_Myr_arr,np.abs(r_shell_pc),s=1,color='paleturquoise',label='$80^\mathrm{th}$ percentile of '+r'$-\nabla u$')
            time_Myr_arr,r_shell_pc = get_large_falls(u_cgs,85,time_Myr,x_pc)
            ax.scatter(time_Myr_arr,np.abs(r_shell_pc),s=1,color='lightskyblue',label='$85^\mathrm{th}$ percentile of '+r'$-\nabla u$')
            ...
            ax.scatter(time_Myr_arr,np.abs(r_shell_pc),s=1,color='mistyrose',label='$80^\mathrm{th}$ percentile of '+r'$-\nabla \rho$')
            time_Myr_arr,r_shell_pc = get_large_falls(rho_cgs,85,time_Myr,x_pc)
            ax.scatter(time_Myr_arr,np.abs(r_shell_pc),s=1,color='lightsalmon',label='$85^\mathrm{th}$ percentile of '+r'$-\nabla \rho$')
...
plt.savefig('shellrad_evol_adiabatic.pdf',format='pdf')
plt.savefig('shellrad_evol_adiabatic.png')
plt.show()
