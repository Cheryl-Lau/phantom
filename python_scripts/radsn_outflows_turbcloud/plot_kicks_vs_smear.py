...
def plot_change_in_r(ax,sn_loc,t_sn,sdf1,sdf2):
    ...
    ax.text(1e-2, 1e+1, r'$t_\mathrm{SN} = $'+str(round(t_sn,2))+' Myr', color='black')
...
def plot_change_in_rho(ax,sn_loc,t_sn,sdf1,sdf2):
    ...
    ax.set_xlim([1e-25,1e-14])
    ax.set_ylim([1e-25,1e-12])

    rholine = np.logspace(-25,-14,100)
    ...
    ax.text(1e-24, 6e-14, r'$t_\mathrm{SN} = $'+str(round(t_sn,2))+' Myr', color='black')

    return pcm



fig, axes = plt.subplots(nrows=2, ncols=3, subplot_kw=dict(frameon=True), figsize=(12,6))
...
ax1.set_title('Semi-confined SN (excl. envelope)')
...
time1, sdf1, sdf1_sinks = read_dump('ff_smear/ffsmear_00000')
time2, sdf2, sdf2_sinks = read_dump('ff_smear/ffsmear_02000')
t_sn = time2 - time1
sn_loc = [0,0,0]
...
ax3.set_title(r'FF - $\rho_\mathrm{smear}$')
...
time2, sdf2, sdf2_sinks = read_dump('turbff_smear/turbffsmear_02000')
t_sn = time2 - time1
sn_loc = [0,0,0]
...
ax5.set_title(r'Turb FF - $\rho_\mathrm{smear}$')


fig.tight_layout()

plt.savefig('sn_kicks_vs_smear.png',dpi=200)
plt.show()
